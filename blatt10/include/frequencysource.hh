#ifndef FREQUENCYSOURCE_HH_SVFCKHQ1
#define FREQUENCYSOURCE_HH_SVFCKHQ1
...
class StreamLetterSource {
 public:
  ...
  StreamLetterSource(Stream& stream) : _stream(stream) {}
...
class StreamWordSource {
 public:
  ...
  StreamWordSource(Stream& stream) : _stream(stream) {}

  // Get the next data item from the source.
  Item next() {
    std::string s;
    _stream >> s;
    if (not _stream) return Item("", false);
...
#endif /* end of include guard: FREQUENCYSOURCE_HH_SVFCKHQ1 */
